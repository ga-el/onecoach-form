...
export const POST: import('astro').APIRoute = async ({ request }) => {
  ...
  try {
    ...
    // Crear HTML con mejor formato y diseño
    let htmlContent = `
      <!DOCTYPE html>
      <html>
      <head>
      ...
      <body>
        <div class="header">
          ...
          <p>Fecha de recepción: ${new Date().toLocaleString('es-MX')}</p>
        </div>
        <div class="container">
    `;

    // Organizar los campos por categorías
    ...
    htmlContent += `
        ...
        <div style="text-align: center; padding: 20px; background-color: #f8fafc; color: #666;">
          ...
          <p>© ${new Date().getFullYear()} ONE COACH DAÑOS - Sistema de Solicitudes</p>
        </div>
      </body>
      </html>
    `;
    ...
    // Intentar con diferentes configuraciones para mejorar entrega
    const emailResponse = await resend.emails.send({
      ...
      text: `Nueva solicitud de seguro empresarial recibida el ${new Date().toLocaleString('es-MX')}. Ver detalles en la versión HTML de este email.`
    });
